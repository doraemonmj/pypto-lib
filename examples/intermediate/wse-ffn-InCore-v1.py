...
def build_wse_ffn_program(
    ...
    @pl.program
    class WseFFN:
        # Phase-1 InCore kernel: one (batch_tile, ffn_out_chunk) block per core.
        ...
        def kernel_gate_up(
            ...
            b0 = (idx // FFN_OUT_BLOCKS) * BATCH_TILE
            o0 = (idx %  FFN_OUT_BLOCKS) * FFN_OUT_CHUNK
        ...
        # Phase-2 InCore kernel: one (batch_tile, hidden_out_chunk) block per core.
        @pl.function(type=pl.FunctionType.InCore)
        def kernel_down(
            self,
            ...
            idx = pl.tile.get_block_idx()
            b0 = (idx // HIDDEN_BLOCKS) * BATCH_TILE
            d0 = (idx %  HIDDEN_BLOCKS) * K_CHUNK
        ...
        def wse_ffn(
            ...
            # Phase 1: SPMD dispatch across (batch_tile × ffn_out_block) grid.
            with pl.spmd(SPMD_BLOCKS * FFN_OUT_BLOCKS):
                ffn_scratch = self.kernel_gate_up(post_norm, w_gate, w_up, ffn_scratch)

            # Phase 2: SPMD dispatch across (batch_tile × hidden_out_block) grid.
            with pl.spmd(SPMD_BLOCKS * HIDDEN_BLOCKS):
